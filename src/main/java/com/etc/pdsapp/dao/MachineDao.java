...
public class MachineDao {
        ...
        public  ObservableList<Machine> getAllMachines() {
            ...
            String query = """
            SELECT machine_id, machine_name, stage_id
            FROM pds.dbo.machines
         ...
         public  boolean insertMachine(Machine machine) {
                String query = """
                INSERT INTO pds.dbo.machines (machine_name,stage_id) VALUES (?,?)
                """;
                try (Connection con = DbConnect.getConnect();
                     PreparedStatement ps = con.prepareStatement(query)) {
         ...
         public  boolean updateMachine(Machine machine) {
                String query = """
                UPDATE pds.dbo.machines
                SET machine_name = ? , stage_id = ?
                WHERE machine_id = ?
                """;
                try (Connection con = DbConnect.getConnect();
                     PreparedStatement ps = con.prepareStatement(query)) {
         ...
         public  boolean deleteMachine(int machineId) {
                String query = """
                DELETE FROM pds.dbo.machines WHERE machine_id = ?
                """;
                try (Connection con = DbConnect.getConnect();
                     PreparedStatement ps = con.prepareStatement(query)) {
        ...
        public  ObservableList<Machine> getMachinesByStageId(int stageId) {
            ObservableList<Machine> list = FXCollections.observableArrayList();
        String query = """
        SELECT machine_id, machine_name , stage_id
        FROM pds.dbo.machines
        ...
        public  int getMachinesCount() {
            String query = """
            SELECT COUNT(*) AS total FROM pds.dbo.machines
            """;
            int count = 0;
        ...
        public  ObservableList<String> getAllMachineNames() {
